      ...
      <main>
        <section class="notification-form">
          ...
          <div class="form-group">
            ...
            <div id="titleError" class="error-message">Title must start with capital letter and use proper spacing</div>
          </div>
          
          <div class="form-group">
            <label for="message">Notification Message</label>
            <textarea id="message" class="form-control" rows="4" placeholder="Enter detailed message here..." maxlength="300"></textarea>
            <div id="messageError" class="error-message">Message must start with capital letter and use proper spacing</div>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      async function loadNotifications() {
        try {
          const response = await fetch('http://98.80.67.100:3013/api/notifications');
      ...
      // Input validation for specific field
      function validateInput(field, errorElement) {
        const isValid = isValidInput(field.value.trim());
        errorElement.style.display = isValid ? 'none' : 'block';
        updateSendButtonState();
      }
      
      // Update send button state based on both fields
      function updateSendButtonState() {
        const titleValid = isValidInput(titleInput.value.trim());
        const messageValid = isValidInput(messageInput.value.trim());
      ...
      async function sendNotification() {
        const title = cleanInput(titleInput.value.trim());
      ...
      // Event listeners with focus tracking
      titleInput.addEventListener('focus', () => activeField = 'title');
      messageInput.addEventListener('focus', () => activeField = 'message');
